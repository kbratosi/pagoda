...
def main():
    ...
    parser.add_argument("ref_batch", help="path to data batch npz file")
    ...
    ref_acts, data = evaluator.read_activations(args.ref_batch)
    print(ref_acts[0].shape, ref_acts[1].shape)
    mu = np.mean(ref_acts[0], axis=0)
    sigma = np.cov(ref_acts[0], rowvar=False)
    mu_spatial = np.mean(ref_acts[1], axis=0)
    sigma_spatial = np.cov(ref_acts[1], rowvar=False)
    np.random.shuffle(data)
    np.savez('/data/NinthArticleExperimentalResults/ImageNet32/author_ckpt/VIRTUAL_imagenet32_labeled.npz',
            data[:10000],
             mu=mu, sigma=sigma,
             mu_s=mu_spatial, sigma_s=sigma_spatial)
...
class Evaluator:
    ...
    def read_activations(self, npz_path: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        return self.compute_activations(batches), batches
