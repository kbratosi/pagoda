...
class MappingNetwork(torch.nn.Module):
    def __init__(self,
        ...
        num_layers      = 3,        # Number of mapping layers.
        num_init_layers = 3,
        ...
        self.num_init_layers = num_init_layers
        ...
        print(f'loaded imagenet embeddings from {embed_path}: {self.embed}, {self.embed.embedding_dim}')
        if rand_embedding:
        ...
        # Construct layers.
        self.embed_proj = FullyConnectedLayer(self.embed.embedding_dim, z_mid_dim, activation='lrelu') if self.c_dim > 0 else None

        features = [self.z_dim] * self.num_init_layers + [z_mid_dim] + [self.w_dim] * (self.num_layers - 1)

        for idx, in_features, out_features in zip(range(self.num_init_layers + num_layers), features[:-1], features[1:]):
            if idx == self.num_init_layers:
                layer = FullyConnectedLayer(in_features + (z_mid_dim if self.c_dim > 0 else 0), out_features, activation='lrelu', lr_multiplier=lr_multiplier)
            else:
    ...
    def forward(self, z, c, truncation_psi=1.0, truncation_cutoff=None):
        ...
        #print("input size: ", (x ** 2).mean())
        #x = x * (x.square().mean(1, keepdim=True) + 1e-8).rsqrt()
        #print("normalized input size: ", (x ** 2).mean())
        #print(x.shape)
        # Execute layers.
        for idx in range(self.num_init_layers + self.num_layers - 1):
            #print(x.shape)
            x = getattr(self, f'fc{idx}')(x)
            if idx == self.num_init_layers - 1 and self.c_dim > 0:
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        conv_clamp          = None,      # Clamp the output to [-X, +X], None = disable clamping.
    ):
        super().__init__()
        self.w_dim = w_dim
    ...
    def forward(self, x, w, noise_mode='random', force_fp32=False):
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs,                 # Arguments for SynthesisLayer.
        ...
        print("get layer specs cutoffs: ", cutoffs)
        print("get layer specs stopbands: ", stopbands)
        print("get layer specs sampling_rates: ", sampling_rates)
        print("get layer specs half_widths: ", half_widths)
        print("get layer specs sizes: ", sizes)
        print("get layer specs channels: ", channels)
        ...
        for idx in range(self.num_layers + 1):
            ...
            layer = SynthesisLayer(
                ...
                in_half_width=half_widths[prev], out_half_width=half_widths[idx],# conv_kernel=1,
                **layer_kwargs)
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.mapping = MappingNetwork(z_dim=z_dim, z_mid_dim=z_mid_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws,
                                      num_init_layers=num_init_layers, **mapping_kwargs)
...
class SuperresGenerator(torch.nn.Module):
    def __init__(
        ...
        **synthesis_kwargs,
        ...
        self.mapping = MappingNetwork(z_dim=z_dim, z_mid_dim=z_mid_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.synthesis.num_ws,
                                      num_init_layers=num_init_layers)
        ...
        self.w_dim = w_dim
        self.img_channels = img_channels
        self.channel_base = self.synthesis.channel_base
        self.channel_max = self.synthesis.channel_max
        self.margin_size = self.synthesis.margin_size
        self.last_stopband_rel = self.synthesis.last_stopband_rel
        self.num_critical = self.synthesis.num_critical
        self.num_fp16_res = self.synthesis.num_fp16_res
        self.conv_kernel = self.synthesis.conv_kernel
        self.use_radial_filters = self.synthesis.use_radial_filters
        ...
        self.img_resolution = img_resolution * up_factor
