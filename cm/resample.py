...
def create_named_schedule_sampler(args, name, num_timesteps):
    ...
    elif name == "uniform_decoder":
        return UniformSampler_decoder(num_timesteps)
    elif name == "loss-second-moment":
        return LossSecondMomentResampler(num_timesteps)
    elif name == "lognormal":
        return LogNormalSampler(args.p_mean, args.p_std)
...
class ScheduleSampler(ABC):
    ...
    def sample(self, batch_size, device):
        return th.rand(batch_size, device=device), None

class UniformSampler_decoder(ScheduleSampler):
...
class UniformSampler:
    ...
    def sample(self, bs, device, num_heun_step=1):
        sigmas = th.rand(bs, device=device) * (1. - self.eps) + self.eps
