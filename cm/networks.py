...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        training_mode='', linear_probing=False, encoder=False,
    ):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        # Decoder.
        #if self.encoder:
        #    self.lin = Linear(in_features=int(model_channels * channel_mult[-1] *
        #                                      (img_resolution / 2 ** (len(channel_mult) - 1))
        #                                      * (img_resolution / 2 ** (len(channel_mult) - 1))),
        #                                    out_features=img_resolution * img_resolution * in_channels * 2,
        #                                                    **init)
        #else:
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            for idx in range(num_blocks + 1):
                #cin = cout if self.encoder else cout + skips.pop()
                cin = cout + skips.pop()
                cout = model_channels * mult
    ...
    def forward(self, x, noise_labels, class_labels=None):
        ...
        #if self.encoder:
        #    return self.lin(x.view(x.shape[0],-1)).view(x.shape[0],2*self.in_channels, self.img_resolution, self.img_resolution) - 1.
        # Decoder.
        aux = None
        tmp = None
        #if self.encoder:
        #    print("t: ", noise_labels[0].item())
...
class EDMPrecond_CTM(torch.nn.Module):
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        if teacher:
            ...
            if arch in ['ddpmpp', 'ncsnpp']:
                ...
                self.model = globals()[model_type](img_resolution=img_resolution, in_channels=img_channels,
                                                   out_channels=img_channels * 2 if encoder else img_channels, label_dim=label_dim,
                                                   ...
                                                   encoder=encoder, model_channels=model_channels, channel_mult=channel_mult,
    ...
    def forward(self, rescaled_x, t=None, encoder=False, **model_kwargs):
        ...
        if t == None and encoder:
            t = self.sigma_min * torch.ones((rescaled_x.shape[0]), device=rescaled_x.device)

        t = t.log() / 4
        F_x = self.model(rescaled_x.to(dtype), t.flatten(), class_labels=class_labels)
        return F_x
