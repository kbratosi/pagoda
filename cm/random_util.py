...
def get_generator(generator, num_samples=0, seed=0, dwt=False):
    if generator == "dummy":
        return DummyGenerator(dwt)
    elif generator == "determ":
        return DeterministicGenerator(num_samples, seed, dwt)
...
class DummyGenerator:
    def __init__(self, dwt=False):
        ...
        if dwt:
            ...
            self.dwt_forward = DWTForward(J=1, mode='zero', wave='haar').to(dist_util.dev())

    def randn(self, *args, **kwargs):
        if self.dwt:
            return self.dwt_forward(th.randn(*args, **kwargs))[0]
        else:
    ...
    def randn_like(self, *args, **kwargs):
        if self.dwt:
            return self.dwt_forward(th.randn_like(*args, **kwargs))[0]
        else:
...
class DeterministicGenerator:
    ...
    def __init__(self, num_samples, seed=0, dwt=False):
    ...
    def randn(self, *size, dtype=th.float, device="cpu"):
        ...
        if self.dwt:
            return self.dwt_forward(th.randn(*global_size, generator=generator, dtype=dtype, device=device)[indices])[0]
        else:
    ...
    def randn_like(self, tensor):
        ...
        if self.dwt:
            return self.dwt_forward(self.randn(*size, dtype=dtype, device=device))[0]
        else:
